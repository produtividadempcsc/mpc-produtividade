...
if 'processo_para_editar_id' in st.session_state:
    ...
    ui_utils.display_favoritos_expander(None, user_id, 'Pages/Processos_MPC.py') # DB arg removed or ignored in refactored utils
    ...
    if not paginated_items:
        ...
        processos_favoritos_ids = {f['id_processo'] for f in get_user_favorites(st.session_state.user_id)}
        ...
        for item in paginated_items:
            ...
            with st.expander("📋 Ver detalhes e ações", expanded=False):
                ...
                with action_cols[0]:
                    ...
                    with icon_col1:
                        is_favorito = pid in processos_favoritos_ids
                        star_icon = "⭐" if is_favorito else "☆"
                        if st.button(star_icon, key=f"fav_chefe_{pid}", help="Marcar como favorito"):
                            # toggle logic
                            toggle_process_favorite(st.session_state.user_id, pid, is_favorito)
                            if is_favorito:
                                # usuario_logado.processos_favoritos.remove(processo_ref)
                                st.toast(f"Processo {pnumero} removido dos favoritos.")
                            else:
                                st.toast(f"Processo {pnumero} adicionado aos favoritos.")
                            # db.commit()
                            st.rerun()
                    with icon_col2:
