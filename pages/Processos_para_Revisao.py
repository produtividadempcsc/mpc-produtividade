        ...
        if not processos_ordenados:
            ...
            for item in processos_ordenados:
                ...
                with st.expander("📋 Ver detalhes e ações", expanded=False):
                    ...
                    with action_cols[0]:
                        ...
                        if st.button(star_icon, key=f"fav_rev_{p_id}", help="Marcar como favorito"):
                             toggle_process_favorite(st.session_state.user_id, p_id)
                             # Optimistic update or rerun
                             st.rerun()

                    with action_cols[1]:
                        if st.button("🔍 Analisar", key=f"analisar_rev_{p_id}", use_container_width=True):
                    ...
                    with action_cols[4]:
                        if st.button("📜 Histórico", key=f"hist_rev_{p_id}", use_container_width=True):
            ...
            if processos_ordenados:
                ...
                <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 3px 15px rgba(0,0,0,0.1); text-align: center;">
                    <h4 style="color: var(--primary-color); margin-bottom: 15px;">📊 Relatório Detalhado</h4>
                    <p>Gere um relatório em PDF com todos os processos filtrados</p>
                </div>
                """, unsafe_allow_html=True)
                ...
                for item in processos_ordenados:
                    p = item['processo']
                    status_cor = "Atrasado" if p.get('status_chefe') == "Revisão Atrasada" else "No Prazo"
                    
                    # Safe date formatting
                    dt_atrib = p.get('data_atribuicao_servidor')
                    dt_conc = p.get('data_conclusao_servidor')
                    dt_conc_chefe = p.get('data_conclusao_chefe')
                    
                    dados_relatorio.append({
                        'status_servidor': status_cor,
                        'processo_numero': p.get('processo_numero'),
                        'servidor_responsavel_nome': item['servidor_nome'],
                        'data_atribuicao_servidor': date.fromisoformat(dt_atrib).strftime('%d/%m/%Y') if dt_atrib else '',
                        'prazo_servidor_aplicado': p.get('prazo_servidor_aplicado'),
                        'data_conclusao_servidor': date.fromisoformat(dt_conc).strftime('%d/%m/%Y') if dt_conc else '',
                        'data_atribuicao_chefe': date.fromisoformat(dt_conc).strftime('%d/%m/%Y') if dt_conc else '',
                        'prazo_chefe_aplicado': p.get('prazo_chefe_aplicado'),
                        'data_conclusao_chefe': date.fromisoformat(dt_conc_chefe).strftime('%d/%m/%Y') if dt_conc_chefe else ''
                    })
                df_relatorio_detalhado = pd.DataFrame(dados_relatorio)
                
                st.download_button(
                    label="📄 Gerar Relatório Detalhado em PDF",
                    data=relatorios.gerar_relatorio_detalhado(df_relatorio_detalhado),
                    file_name=f"relatorio_detalhado_revisao_{datetime.now().strftime('%Y%m%d')}.pdf",
