...
# 7. Percentual Prazo Global - Métricas 3 e 7 do Relatório
...
# 8. Tempo Médio Global - Métricas 2 e 6 do Relatório
...
with k5: st.metric("📂 Acervo Servidores (Pendentes)", kpi_acervo_servidores, help="Métrica 4 do Relatório Mensal — processos não concluídos pelos servidores")
with k6: st.metric("📋 Acervo Chefes (Pend. Revisão)", kpi_acervo_chefes, help="Métrica 8 do Relatório Mensal — processos não revisados pelos chefes")

with k7: st.metric("⏱️ Tempo Médio (Servidores)", f"{tm_serv:.1f} dias", help="Métrica 2 do Relatório Mensal — Média Global MPC")
with k8: st.metric("⏱️ Tempo Médio de Revisão (Chefes)", f"{tm_chefe:.1f} dias", help="Métrica 6 do Relatório Mensal — Média Global MPC")
with k9: st.metric("🎯 % Conclusão no Prazo (Serv)", f"{pct_prazo_serv:.1f}%", help="Métrica 3 do Relatório Mensal — Média Global MPC")

with k10: st.metric("🎯 % Revisão no Prazo (Chefes)", f"{pct_prazo_chefe:.1f}%", help="Métrica 7 do Relatório Mensal — Média Global MPC")
...
# --- INDICADOR 1: Produção por Ano e Mês ---
st.markdown("### 📅 Indicador 1 – Processos Produzidos por Ano e Mês")
st.caption("Consolidação da quantidade de processos e procedimentos produzidos no MPC/SC, por ano e por mês.")

if not df_concluidos_servidor.empty:
    df_ind1 = df_concluidos_servidor.copy()
    df_ind1['ano'] = df_ind1['data_conclusao_servidor'].dt.year
    df_ind1['mes_num'] = df_ind1['data_conclusao_servidor'].dt.month
    df_ind1['mes_dt'] = df_ind1['data_conclusao_servidor'].dt.to_period('M').dt.to_timestamp()
    df_ind1['mes'] = df_ind1['data_conclusao_servidor'].apply(formatar_mes)
    ...
    fig_ind1 = px.bar(
        ...
        labels={'mes': 'Mês', 'Quantidade': 'Processos Concluídos', 'ano': 'Ano'}
    )
    ...
    fig_ind1.update_layout(
        ...
        xaxis_title='Mês/Ano',
        yaxis_title='Processos Concluídos'
    )
...
# --- INDICADOR 2: Produção por Procuradoria ---
st.markdown("### 🏛️ Indicador 2 – Processos Produzidos por Procuradoria de Contas")
st.caption("Consolidação da quantidade de processos e procedimentos produzidos no MPC/SC, por Procuradoria de Contas no período selecionado.")

if not df_concluidos_servidor.empty:
    grp_proc = df_concluidos_servidor.groupby('procurador_nome').agg(
        concluidos=('id', 'count'),
        no_prazo=('no_prazo_servidor', 'sum')
    ).reset_index()
    grp_proc['pct_prazo'] = (grp_proc['no_prazo'] / grp_proc['concluidos'] * 100).fillna(0)
    grp_proc = grp_proc.sort_values('concluidos', ascending=True)

    fig_ind2 = px.bar(
        grp_proc, x='concluidos', y='procurador_nome', orientation='h',
        text='concluidos',
        color='pct_prazo', color_continuous_scale='RdYlGn', range_color=[0, 100],
        labels={'concluidos': 'Processos Concluídos', 'procurador_nome': 'Procuradoria', 'pct_prazo': '% no Prazo'}
    )
    fig_ind2.update_traces(textposition='outside')
    fig_ind2.update_layout(
        plot_bgcolor='rgba(0,0,0,0)',
        yaxis={'categoryorder': 'total ascending'},
        xaxis_title='Processos Concluídos',
...
# --- INDICADOR 3: Acervo (Estoque) por Procuradoria/Mês ---
st.markdown("### 📦 Indicador 3 – Processos em Estoque por Procuradoria de Contas")
st.caption("Consolidação da quantidade de processos em estoque por Procuradorias de Contas do MPC/SC por mês.")
...
mes_atual_fim_hist = hoje_dt_hist.replace(day=1) + pd.offsets.MonthEnd(0)
dates_to_check_hist = pd.date_range(end=mes_atual_fim_hist, periods=24, freq='ME')

history_proc = []   # Para Indicador 3
...
if len(dates_to_check_hist) > 0:
    ...
    for i, date_ref in enumerate(dates_to_check_hist):
        ...
        # Agrupamento por Procuradoria (Indicador 3)
        if not full_acervo.empty:
            counts_p = full_acervo.groupby('procurador_nome').size().reset_index(name='qtd')
        ...
        # Agrupamento por Chefe
        if not full_acervo.empty:
            counts_c = full_acervo['chefe_gabinete_nome'].value_counts().reset_index()
...
# Exibir Indicador 3 (Procuradoria)
if history_proc:
    df_hist_proc = pd.concat(history_proc)
    fig_ind3 = px.line(
        df_hist_proc, x='data', y='qtd', color='procurador_nome', markers=True,
        labels={'data': 'Mês', 'qtd': 'Processos em Estoque', 'procurador_nome': 'Procuradoria'}
    )
...
if not df_concluidos_servidor.empty:
    tipos_count = df_concluidos_servidor['nome_produto'].value_counts().reset_index()
    ...
    with col_t1:
        fig_ind4a = px.bar(
            ...
            labels={'Quantidade': 'Processos', 'Tipo de Produto': 'Tipo'}
        )
        fig_ind4a.update_traces(textposition='outside')
        fig_ind4a.update_layout(
            plot_bgcolor='rgba(0,0,0,0)',
            yaxis={'categoryorder': 'total ascending'},
            showlegend=False
        )
        st.plotly_chart(fig_ind4a, use_container_width=True)

    with col_t2:
        fig_ind4b = px.pie(
            tipos_count, values='Quantidade', names='Tipo de Produto',
            color_discrete_sequence=px.colors.sequential.RdBu
        )
        fig_ind4b.update_traces(textposition='inside', textinfo='percent+label')
        fig_ind4b.update_layout(plot_bgcolor='rgba(0,0,0,0)')
        st.plotly_chart(fig_ind4b, use_container_width=True)
else:
    st.info("Sem dados de tipos de produto.")

st.markdown("---")

# --- INDICADOR 5: Tempo Médio por Procuradoria ---
st.markdown("### ⏱️ Indicador 5 – Tempo Médio de Produção por Procuradoria de Contas")
st.caption("Apresentação do tempo médio de produção de produtos do MPC por Procuradoria de Contas.")

if not df_concluidos_servidor.empty:
    grp_tm_proc = df_concluidos_servidor.groupby('procurador_nome').agg(
    ...
    fig_ind5 = px.bar(
        ...
        labels={'tempo_medio': 'Tempo Médio (Dias)', 'procurador_nome': 'Procuradoria', 'concluidos': 'Processos'},
