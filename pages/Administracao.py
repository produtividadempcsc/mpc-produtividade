...
# --- ABA LOGS DO SISTEMA ---
with admin_tabs[7]:
    st.subheader("📋 Logs das Edge Functions")
    st.caption("Últimas 100 execuções das funções automáticas (update-statuses, notify-deadlines, auto-backup).")

    try:
        col_f1, col_f2, col_f3 = st.columns([2, 2, 1])
        with col_f1:
            sel_servico = st.selectbox("🔧 Serviço", ["Todos", "update-statuses", "notify-deadlines", "auto-backup"], key="logs_servico")
        with col_f2:
            sel_nivel = st.selectbox("🚦 Nível", ["Todos", "INFO", "AVISO", "ERRO"], key="logs_nivel")
        with col_f3:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("🔄 Atualizar", key="logs_refresh"):
                st.rerun()

        query = supabase.table("sistema_logs").select("*").order("created_at", desc=True).limit(100)
        if sel_servico != "Todos":
            query = query.eq("servico", sel_servico)
        if sel_nivel != "Todos":
            query = query.eq("nivel", sel_nivel)

        resultado = query.execute()
        logs = resultado.data if resultado.data else []

        if not logs:
            st.info("📭 Nenhum log encontrado para os filtros selecionados.")
        else:
            icone_nivel   = {"INFO": "🟢", "AVISO": "🟡", "ERRO": "🔴"}
            icone_servico = {"update-statuses": "🔄", "notify-deadlines": "📧", "auto-backup": "💾"}

            for log_entry in logs:
                nivel    = log_entry.get('nivel', 'INFO')
                servico  = log_entry.get('servico', '')
                mensagem = log_entry.get('mensagem', '')
                detalhes = log_entry.get('detalhes')
                raw_ts   = log_entry.get('created_at', '')

                try:
                    from datetime import timezone
                    dt_obj = datetime.fromisoformat(raw_ts.replace('Z', '+00:00'))
                    dt_local = dt_obj.astimezone(timezone(timedelta(hours=-3)))
                    data_fmt = dt_local.strftime('%d/%m/%Y %H:%M:%S')
                except Exception:
                    data_fmt = raw_ts[:19].replace('T', ' ')

                with st.container(border=True):
                    col_a, col_b = st.columns([4, 1])
                    with col_a:
                        icon_n = icone_nivel.get(nivel, "⚪")
                        icon_s = icone_servico.get(servico, "⚙️")
                        st.markdown(f"{icon_n} **{nivel}** &nbsp;|&nbsp; {icon_s} `{servico}` &nbsp;|&nbsp; {mensagem}")
                    with col_b:
                        st.caption(data_fmt)
                    if detalhes:
