    ...
    if favoritos_procurador:
        with st.expander(f"⭐ Meus Favoritos ({len(favoritos_procurador)})", expanded=False):
            ...
            for p in favoritos_procurador:
                ...
                with fav_col2:
                    if st.button("✅ Marcar como Finalizado", key=f"fav_finalizar_{p_id}", use_container_width=True, type="primary"):
                ...
                with fav_col3:
                    if st.button("↩️ Devolver para o Gabinete", key=f"fav_devolver_{p_id}", use_container_width=True):
                ...
                with fav_col4:
                    ...
                    if st.button(button_label, key=f"fav_comments_{p_id}", use_container_width=True, type=button_type):
                ...
                with fav_col5:
                    if st.button("📜 Ver Histórico", key=f"fav_hist_{p_id}", use_container_width=True):
    ...
    if not processos_com_procurador:
        ...
        for p in processos_com_procurador:
            ...
            with st.expander("📋 Ver detalhes e ações"):
                ...
                with b_col2:
                    if st.button("✅ Marcar como Finalizado", key=f"procurador_concluiu_{p['id']}", use_container_width=True, type="primary"):
                ...
                with b_col3:
                    if st.button("↩️ Devolver para o Gabinete", key=f"procurador_devolve_{p['id']}", use_container_width=True):
