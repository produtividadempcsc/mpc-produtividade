...
# Chave da API Gemini recuperada dos segredos (lazy-load para evitar st.error no import-time)
GEMINI_API_KEY = None
...
def get_gemini_api_key():
    ...
    if GEMINI_API_KEY is None:
        try:
            ...
            GEMINI_API_KEY = ""
            print("AVISO: Chave da API Gemini não encontrada em .streamlit/secrets.toml")
...
from supabase_client import select_first, update_by_id, QueryBuilder
...
def login_user(login, password):
    ...
    try:
        ...
        if user and bcrypt.checkpw(password.encode('utf-8'), user['senha_hash'].encode('utf-8')):
            ...
            # Fix #12: Passa o user já carregado em vez de buscar de novo
            post_login_setup(user)
            return True
        return False
...
def post_login_setup(user):
    """
    Configura dados adicionais da sessão após um login bem-sucedido.
    Fix #12: Recebe o objeto user já carregado para evitar query duplicada.
    """
    try:
        if user:
            # Fix #5: Usar now_brazil em vez de datetime.now()
            from utils.timezone import now_brazil
            update_by_id("usuarios", user['id'], {"ultimo_acesso": now_brazil().isoformat()})
