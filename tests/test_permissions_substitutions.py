...
from supabase_client import insert, update_by_id, delete_by_id
...
class TestPermissionsSubstitutions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        import uuid
        chefe_data = {
            "nome_completo": f"Chefe Teste {uuid.uuid4().hex[:8]}",
            "login": f"chefe.test.{uuid.uuid4().hex[:8]}",
            "senha_hash": "hash_fake",
            "perfil": "Chefe de Gabinete",
            "email": f"chefe.{uuid.uuid4().hex[:8]}@test.com",
            "ativo": True
        }
        ...
        # 2.5 Criar Tipo de Produto
        from datetime import datetime
        produto_data = {
            "nome_produto": f"TEST-SUB-PROD-{uuid.uuid4().hex[:6]}",
            "prazo_servidor": 5,
            "prazo_chefe": 2,
        ...
        self.produto = insert("tipos_produto", produto_data)
        self.produto_id = self.produto['id']
        self.created_data["produtos"] = [self.produto_id]
        
        # 3. Criar Processo
        processo_data = {
            "processo_numero": f"TEST-SEC-{uuid.uuid4().hex[:8]}",
            "id_tipo_produto": self.produto_id, 
    ...
    def tearDown(self):
        ...
        for prod_id in self.created_data.get("produtos", []):
            delete_by_id("tipos_produto", prod_id)
