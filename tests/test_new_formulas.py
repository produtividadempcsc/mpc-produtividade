...
# We need to mock calculate_net_work_days, calculate_net_duration_calendar, calculate_due_date
# But analytics_utils imports them from db_compat.
# Let's mock them in analytics_utils.utils
...
class TestNewFormulas(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Mock calculate_net_work_days: returns (end - start).days - 2 (simulating weekend)
        self.original_net_work = utils.calculate_net_work_days
        utils.calculate_net_work_days = lambda s, e, u: max(0, (e - s).days - 2)
        
        # Mock calculate_net_duration_calendar: returns (end - start).days + 1 - 0 (no leaves) - suspension
        self.original_net_calendar = utils.calculate_net_duration_calendar
        utils.calculate_net_duration_calendar = lambda s, e, u, susp: max(0, (e - s).days + 1 - susp)
        
        # Mock calculate_due_date: returns start + prazo (simple)
        self.original_due_date = utils.calculate_due_date
        # Note: Argument names must match those used in analytics_utils.py call
        utils.calculate_due_date = lambda start_date, prazo_dias, tipo_contagem, id_usuario, dias_suspensos: start_date + timedelta(days=prazo_dias+dias_suspensos)

    def tearDown(self):
        # Restore originals
        utils.calculate_net_work_days = self.original_net_work
        utils.calculate_net_duration_calendar = self.original_net_calendar
        utils.calculate_due_date = self.original_due_date
