...
class TestPrazosNotificacoes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Buscar IDs de teste
        # 1. Criar Servidor e Chefe Fake para os testes
        import uuid
        from supabase_client import insert
        
        serv_data = {
            "nome_completo": f"Servidor Teste {uuid.uuid4().hex[:8]}",
            "login": f"serv.test.{uuid.uuid4().hex[:8]}",
            "senha_hash": "hash",
            "perfil": "Servidor",
            "email": f"serv.{uuid.uuid4().hex[:8]}@test.com",
            "ativo": True
        }
        self.servidor = insert("usuarios", serv_data)
        self.servidor_id = self.servidor['id']
        self.created_users.append(self.servidor_id)
        ...
        self.created_users.append(self.chefe_id)
