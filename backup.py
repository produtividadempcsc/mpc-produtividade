...
TABLE_NAMES = [
    ...
    'anexos_processos', 'chefe_subordinado_chefe', 'comentario_lido',
    'comentarios', 'notificacoes', 'processo_favoritos'
]
...
RESTORE_ORDER = [
    ...
    'comentarios', 'processo_historico', 'anexos_processos',
...
def executar_backup_automatico_e_enviar_email():
    ...
    """
    print(f"[{datetime.now()}] JOB INICIADO: Verificação de backup automático.")
    try:
        ...
        if freq == "Desativado":
            print(f"[{datetime.now()}] INFO: Backup automático desativado. Job ignorado.")
            return
        ...
        if not needs_backup:
            print(f"[{datetime.now()}] INFO: Backup não é necessário hoje. Próxima verificação agendada.")
            return

        print(f"[{datetime.now()}] AÇÃO: Backup necessário. Gerando arquivo...")
        file_path = backup_local_excel()

        if not file_path:
            print(f"[{datetime.now()}] ERRO: A criação do arquivo de backup local falhou.")
            return

        set_config('data_ultimo_backup', datetime.now().isoformat())
        
        print(f"[{datetime.now()}] SUCESSO: Backup local criado em '{file_path}'.")

        destinatario = email_backup
        if not destinatario:
            print(f"[{datetime.now()}] AVISO: E-mail de backup não configurado. O arquivo foi salvo localmente mas não será enviado.")
            return

        print(f"[{datetime.now()}] AÇÃO: Enviando backup para {destinatario}...")
        assunto = f"Backup do Sistema de Produtividade - {datetime.now().strftime('%d/%m/%Y')}"
        corpo = f"Olá,\\n\\nO backup automático ({freq.lower()}) do sistema de produtividade está em anexo.\\n\\nData de geração: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"
        
        success = utils.send_email_notification(destinatario, assunto, corpo, attachment_path=file_path, force_send=True)

        if success:
            print(f"[{datetime.now()}] SUCESSO: Backup enviado por e-mail para {destinatario}.")
        else:
            print(f"[{datetime.now()}] ERRO: Falha ao enviar o e-mail de backup.")

    except Exception as e:
        print(f"[{datetime.now()}] ERRO CRÍTICO no job 'executar_backup_automatico_e_enviar_email': {e}")
        import traceback
        traceback.print_exc()
    finally:
        print(f"[{datetime.now()}] JOB CONCLUÍDO: Verificação de backup automático.")
...
def executar_backup_manual_e_enviar_email(engine=None): # engine param kept for compatibility but ignored
    ...
    """
    print(f"[{datetime.now()}] AÇÃO: Backup manual solicitado.")
    ...
    if not destinatario:
        ...
        print(f"[{datetime.now()}] ERRO: {msg}")
        return False, msg

    print(f"[{datetime.now()}] AÇÃO: Gerando arquivo de backup manual...")
    file_path = backup_local_excel()
    ...
    print(f"[{datetime.now()}] AÇÃO: Enviando backup para {destinatario}...")
    assunto = f"Backup Manual do Sistema de Produtividade - {datetime.now().strftime('%d/%m/%Y')}"
    corpo = f"Olá,\\n\\nO backup manual solicitado do sistema de produtividade está em anexo.\\n\\nData de geração: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"
    
    success = utils.send_email_notification(destinatario, assunto, corpo, attachment_path=file_path, force_send=True)

    try:
        os.remove(file_path)
        print(f"[{datetime.now()}] INFO: Arquivo de backup local '{file_path}' removido após o envio.")
    except OSError as e:
        print(f"[{datetime.now()}] AVISO: Falha ao remover o arquivo de backup local: {e}")

    if success:
        msg = f"Backup manual enviado com sucesso por e-mail para {destinatario}."
        print(f"[{datetime.now()}] SUCESSO: {msg}")
        return True, msg
    else:
