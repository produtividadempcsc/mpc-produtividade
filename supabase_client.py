...
def select_all(table: str, columns: str = "*"):
    ...
    Pagina automaticamente para superar o limite de 1000 linhas do Supabase.
...
def select_where(table: str, column: str, value, columns: str = "*"):
    ...
    try:
        ...
        offset = 0
        while True:
            result = supabase.table(table).select(columns).eq(column, value).range(offset, offset + page_size - 1).execute()
            data = result.data
            if not data:
                break
            all_data.extend(data)
...
def rpc(fn_name: str, params: dict = None):
    """
    Executa uma Remote Procedure Call (Stored Procedure) no Supabase (PostgreSQL).
    
    Args:
        fn_name: Nome da função RPC criada no banco
        params: Dicionário com os parâmetros (ex: {"p_id_servidor": 10})
        
    Returns:
        O retorno da função ou None em caso de erro.
    """
    try:
        # A API oficial python permite supabase.rpc('function_name', {params})
        if params is None:
            params = {}
        result = supabase.rpc(fn_name, params).execute()
        return result.data
    except Exception as e:
        print(f"[SUPABASE] Erro ao executar RPC '{fn_name}': {e}")
        return None
...
class QueryBuilder:
    ...
    def ilike_all_words(self, column: str, search_text: str):
        """
        Busca por proximidade: divide o texto em palavras e aplica um ILIKE
        para cada palavra (AND). Ex: 'PI 2026' → ilike '%PI%' AND ilike '%2026%'.
        Isso permite encontrar 'PI 54-2026' pesquisando 'PI 2026'.
        """
        words = search_text.strip().split()
        for word in words:
            self.ilike(column, f"%{word}%")
        return self
    ...
    def _apply_filters(self, query):
        """Aplica os filtros armazenados em uma query."""
    ...
    def execute(self):
        ...
        para superar o limite de 1000 linhas do Supabase.
        
        Se limit() foi chamado explicitamente, respeita o limite sem paginar.
        
        Returns:
            Lista de dicionários com os resultados
        """
        try:
            # Se o usuário definiu limit/offset explícitos, respeitar (sem paginação automática)
            if self._limit_val is not None or self._offset_val is not None:
                query = supabase.table(self.table_name).select(self._columns)
                query = self._apply_filters(query)
                query = self._apply_order(query)
                if self._limit_val:
            ...
            while True:
                ...
                # Nota: NÃO adicionar order("id") por padrão, pois algumas tabelas
                # (ex: procurador_chefes, gabinete_servidores) não possuem coluna "id".
                # A paginação via range() funciona sem ordenação explícita.
    ...
    def fetch_all(self):
        """
        Alias para execute(). Mantido para compatibilidade.
        execute() já pagina automaticamente.
        
        Returns:
            Lista de dicionários completa
        """
        # Forçar paginação ignorando limit/offset manuais
