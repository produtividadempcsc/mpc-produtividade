...
from repositories.devolucao_repository import registrar_devolucao, concluir_devolucao
from repositories.devolucao_procurador_chefe_repository import concluir_devolucao_procurador_chefe
import ui_utils
...
def display_analise_form(processo_id):
    ...
    with st.form("form_analise"):
        ...
        with col1:
            nova_data_atribuicao = st.date_input("Nova Data de Atribuição", value=today_brazil(), format="DD/MM/YYYY")
        with col2:
        ...
        data_aprovacao = st.date_input("Data de Envio ao Procurador", value=today_brazil(), format="DD/MM/YYYY")
        ...
        if aprovar_e_enviar_procurador:
            concluir_devolucao_procurador_chefe(pid, data_aprovacao)
            updates = {
            ...
            # Notificar procurador sobre processo aprovado
            proc_id = processo.get('id_procurador')
            if proc_id:
                create_notification(
                    proc_id,
                    f"O processo '{p_num}' foi aprovado pelo chefe e enviado para sua análise.",
                    tipo="conclusao",
                    id_processo=pid
                )
            
            ui_utils.set_success_feedback(f"Processo {p_num} aprovado e enviado para o Procurador!", "success")
        ...
        if devolver_button:
            if not observacao_devolucao:
                ...
                updates = {
                    ...
                    "prazo_customizado": True,
                    "observacao_chefe": observacao_devolucao
                }
                
                update_process(pid, updates)
                
                # Registrar devolução na tabela auxiliar
                registrar_devolucao(
                    id_processo=pid,
                    data_devolucao=nova_data_atribuicao,
                    prazo_dias=prazo_adicional,
                ...
                add_process_history(
                    ...
                    f"{observacao_devolucao} | Prazo: {prazo_adicional} dias a partir de {nova_data_atribuicao.strftime('%d/%m/%Y')}"
                )

                # Fix #13: Adicionado timestamp ao comentário de devolução
                ...
                ui_utils.set_success_feedback("Processo devolvido ao servidor com novo prazo.", "warning", "↩️")
                if 'processo_em_analise_id' in st.session_state:
                    del st.session_state['processo_em_analise_id']
                st.rerun()
...
def display_chefe_update_form(processo_id):
    ...
    with st.form("chefe_update_form"):
        data_conclusao = st.date_input("Data de Conclusão", value=today_brazil())
        ...
        if concluir_button:
            concluir_devolucao_procurador_chefe(pid, data_conclusao)
            updates = {
            ...
            ui_utils.set_success_feedback("Processo concluído e enviado para o procurador!", "success")
...
def display_servidor_update_form(processo_id):
    ...
    with st.form("servidor_update_form"):
        ...
        if concluir_button:
            ...
            # Marcar devolução como concluída (se existir)
            concluir_devolucao(pid, data_conclusao)
            ...
            ui_utils.set_success_feedback("Processo concluído e enviado para revisão!", "success")
            if 'processo_para_atualizar_id' in st.session_state:
                del st.session_state['processo_para_atualizar_id']
            st.rerun()
