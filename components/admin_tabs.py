    ...
    with col1:
        with st.expander("➕ Adicionar Produto", expanded=True):
            with st.form("add_prod", clear_on_submit=True):
                ...
                data_criacao = st.date_input("📅 Data de Criação", value=today_brazil(), min_value=date(2000, 1, 1), max_value=date(2099, 12, 31), format="DD/MM/YYYY")
                data_validade = st.date_input("⏰ Data de Validade", value=today_brazil(), min_value=date(2000, 1, 1), max_value=date(2099, 12, 31), format="DD/MM/YYYY")
                ...
                if sel_prod_name:
                    ...
                    if prod_to_edit:
                        with st.form("edit_prod_form"):
                            ...
                            n_data_validade = st.date_input("⏰ Data de Validade", value=current_validade or today_brazil(), min_value=date(2000, 1, 1), max_value=date(2099, 12, 31), format="DD/MM/YYYY")
            ...
            if not produto_nomes:
                ...
                with st.form("alterar_prazo_form", clear_on_submit=True):
                    if produto_selecionado:
                        ...
                        nova_vigencia = st.date_input("📅 Vigência do Novo Prazo", value=today_brazil(), min_value=date(2000, 1, 1), max_value=date(2099, 12, 31), format="DD/MM/YYYY")
                    ...
                    if submitted and produto_selecionado:
                        ...
                        nova_versao_data = {
                            ...
                            "data_criacao": now_brazil().isoformat(),
...
def render_tab_backup():
    ...
    with st.container(border=True):
            ...
            with st.spinner("⏳ Gerando backup completo..."):
                try:
                    ...
                    if generated_path and os.path.exists(generated_path):
                        ...
                        st.session_state['backup_manual_timestamp'] = now_brazil()
                        st.rerun()
                    else:
        ...
        if 'backup_manual_path' in st.session_state and os.path.exists(st.session_state['backup_manual_path']):
            ...
            bkp_time = st.session_state.get('backup_manual_timestamp', now_brazil())
...
def render_tab_relatorios_mensais():
    ...
    try:
        ...
        with st.container(border=True):
            ...
            with col_rep1:
                ...
                if not anos_disponiveis:
                    anos_disponiveis = [now_brazil().year]
            ...
            with col_rep3:
                ...
                if st.button("📄 Gerar Relatório PDF", type="primary", width='stretch', key="btn_rel_mensal"):
        ...
        with st.container(border=True):
            ...
            with col_p4:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("📊 Gerar Relatório do Período", type="primary", width='stretch', key="btn_rel_periodo"):
        ...
        with st.container(border=True):
            ...
            with col_z1:
                ...
                if not anos_lote:
                    anos_lote = [now_brazil().year]
                sel_ano_lote = st.selectbox("Ano para Lote", anos_lote, key="rel_lote_ano")
            
            with col_z2:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("📦 Gerar Todos os Relatórios do Ano (.zip)", type="primary", width='stretch', key="btn_rel_lote"):
    ...
    try:
        ...
        if not proc_options:
            ...
            with col_dates:
                ...
                with c_d1:
                    dt_inicio_corr = st.date_input("Data de Início:", value=today_brazil().replace(day=1), min_value=date(2000, 1, 1), max_value=date(2099, 12, 31), format="DD/MM/YYYY")
                with c_d2:
                    dt_fim_corr = st.date_input("Data de Fim:", value=today_brazil(), min_value=date(2000, 1, 1), max_value=date(2099, 12, 31), format="DD/MM/YYYY")
